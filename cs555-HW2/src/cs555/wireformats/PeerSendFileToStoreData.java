...
public class PeerSendFileToStoreData implements Event {

	private final int type = Protocol.PEER_SEND_FILE_TO_STORE_DATA;
	...
	public PeerSendFileToStoreData(String filename, String key, NodeInformation peer, int numberOfTraces,
			ArrayList<String> traceList, int hopCount, byte[] fileBytes) {
	 ...
	 * @throws IOException 
	 */
	public PeerSendFileToStoreData(byte[] marshalledBytes) throws IOException {
		ByteArrayInputStream baInputStream = new ByteArrayInputStream(marshalledBytes);
		DataInputStream din = new DataInputStream(new BufferedInputStream(baInputStream));
		
		int type = din.readInt();
		
		if (type != Protocol.PEER_SEND_FILE_TO_STORE_DATA) {
			System.out.println("Invalid Message Type for PeerSendFileToStoreData");
			return;
		}
