...
public class StoreDataReadRequestToDiscovery implements Event {

	private final int type = Protocol.STOREDATA_READ_REQUEST_TO_DISCOVERY;
	private NodeInformation storeData;
	private String filename;
	private String key;
	
	public StoreDataReadRequestToDiscovery(NodeInformation storeData, String filename, String key) {
	 ...
	 * @throws IOException 
	 */
	public StoreDataReadRequestToDiscovery(byte[] marshalledBytes) throws IOException {
		ByteArrayInputStream baInputStream = new ByteArrayInputStream(marshalledBytes);
		DataInputStream din = new DataInputStream(new BufferedInputStream(baInputStream));
		
		int type = din.readInt();
		
		if (type != Protocol.STOREDATA_READ_REQUEST_TO_DISCOVERY) {
			System.out.println("Invalid Message Type for StoreDataReadRequestToDiscovery");
			return;
		}
