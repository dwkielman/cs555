...
public class PeerLeftLeafRouteTraceToPeer implements Event {

	private final int type = Protocol.PEER_LEFT_LEAF_ROUTE_TRACE_TO_PEER;
	private String originNodeIdentifier;
	private int numberOfTraces;
	private ArrayList<String> traceIdentifierList;
	
	public PeerLeftLeafRouteTraceToPeer(String originNodeIdentifier, int numberOfTraces,
	 ...
	 * @throws IOException 
	 */
	public PeerLeftLeafRouteTraceToPeer(byte[] marshalledBytes) throws IOException {
		ByteArrayInputStream baInputStream = new ByteArrayInputStream(marshalledBytes);
		DataInputStream din = new DataInputStream(new BufferedInputStream(baInputStream));
		
		int type = din.readInt();
		
		if (type != Protocol.PEER_LEFT_LEAF_ROUTE_TRACE_TO_PEER) {
			System.out.println("Invalid Message Type for PeerLeftLeafRouteTraceToPeer");
			return;
		}
