...
public class ChunkServerNotifyFixSuccessToController implements Event {

	private final int type = Protocol.CHUNKSERVER_NOTIFY_FIX_SUCCESS_TO_CONTROLLER;
	private NodeInformation chunkServer;
	private int chunkNumber;
	private String filename;
	
	public ChunkServerNotifyFixSuccessToController(NodeInformation chunkServer, int chunkNumber, String filename) {
	 ...
	 * @throws IOException 
	 */
	public ChunkServerNotifyFixSuccessToController(byte[] marshalledBytes) throws IOException {
		ByteArrayInputStream baInputStream = new ByteArrayInputStream(marshalledBytes);
		DataInputStream din = new DataInputStream(new BufferedInputStream(baInputStream));
		
		int type = din.readInt();
		
		if (type != Protocol.CHUNKSERVER_NOTIFY_FIX_SUCCESS_TO_CONTROLLER) {
			System.out.println("Invalid Message Type for ChunkServerNotifyFixSuccessToController");
			return;
		}
